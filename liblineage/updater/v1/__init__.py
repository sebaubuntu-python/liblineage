...
class AsyncV1Api:
    @staticmethod
    async def get_device_builds(
        device: str, rom_type: str, incremental_version: str
    ) -> List[Build]:
        """Get the list of builds for a device."""
        json = await AsyncHttpRequests.get_json(
            f"{API_URL}/{device}/{rom_type}/{incremental_version}"
        )
